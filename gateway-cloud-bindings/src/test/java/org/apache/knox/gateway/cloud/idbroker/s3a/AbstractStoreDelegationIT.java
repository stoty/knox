 ...
 */
public abstract class AbstractStoreDelegationIT extends AbstractS3AStoreTest {

  protected static final String YARN_RM = "yarn-rm@EXAMPLE";

  private static final Logger LOG =
      LoggerFactory.getLogger(AbstractStoreDelegationIT.class);
  ...
  protected static Credentials mkTokens(final S3AFileSystem fs)
    ...
    fs.addDelegationTokens("yarn-rm@EXAMPLE", cred);
    return cred;
  }
  ...
  protected static void assertBoundToDT(final S3AFileSystem fs,
    ...
    Optional<S3ADelegationTokens> integration = fs.getDelegationTokens();
    Token<AbstractS3ATokenIdentifier> token
        = integration.flatMap(S3ADelegationTokens::getBoundDT)
        .orElseThrow(() -> new AssertionError("not bound: " + integration));
