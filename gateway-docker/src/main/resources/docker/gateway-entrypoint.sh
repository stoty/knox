...
# - CDP_CERT - (optional) the location of a file containing the CDP cert
...
# Import cdp certs one by one
if [[ -n $CDP_CERT ]] && [[ -f ${CDP_CERT} ]]
then
  echo "Importing CDP certs."
  importMultipleCerts "$CDP_CERT"
fi

export KNOX_GATEWAY_DBG_OPTS="${KNOX_GATEWAY_DBG_OPTS} -Djavax.net.ssl.trustStore=${KEYSTORE_DIR}/truststore.jks"
